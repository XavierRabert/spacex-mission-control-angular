...
import { RocketCard } from './components/launch-card/rocket-card';
import { RocketsService } from '@features/rockets/services/rockets.service';

@Component({
  selector: 'spx-rockets-list',
  imports: [RocketCard, Title, Pagination, TechBorderDirective],
  templateUrl: './rockets-list.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RocketsList {
  private _rocketsService = inject(RocketsService);
  private _router = inject(Router);

  public rockets = this._rocketsService.rockets;
  public currentPaginationKey = this._rocketsService.currentPaginationKey;
  ...
  public cards = viewChildren(RocketCard);
